...
-- ══════════════════════════════════════════════════════════════════════════════
-- BUILD ALL GUI FIRST (no waiting)
-- ══════════════════════════════════════════════════════════════════════════════
...
hpSkull.Size               = UDim2.new(0, 28, 1, 0)
...
RunService.Heartbeat:Connect(function(dt)
	...
	camera.CFrame = CFrame.lookAt(
		Vector3.new(math.cos(cameraAngle)*30, 15, math.sin(cameraAngle)*30),
		Vector3.new(0, 5, 0)
	)
end)
...
RunService.Heartbeat:Connect(function()
	...
	for _, obj in ipairs(workspace:GetChildren()) do
		...
		if n == "skibidi toilet" or n == "big skibidi toilet" then
			...
			if h and h.Health > 0 then count += 1 end
		end
	end
...
-- ══════════════════════════════════════════════════════════════════════════════
-- CONNECT REMOTE EVENTS (wait in background — GUI already visible)
-- ══════════════════════════════════════════════════════════════════════════════
task.spawn(function()
	...
	if not gameStarted then
		warn("[LocalScript] WaveManager never created RemoteEvents — is it in ServerScriptService?")
		return
	end
	...
	local existing = player:FindFirstChild("Tokens")
	if existing then
		existing.Changed:Connect(updateTokens)
		updateTokens()
	end
	...
	local function showFallBackAlert()
		...
		task.delay(3.5, function()
			...
			TweenService:Create(alertText,    TweenInfo.new(0.6), {TextTransparency = 1}):Play()
			TweenService:Create(alertSub,     TweenInfo.new(0.6), {TextTransparency = 1}):Play()
			TweenService:Create(alertBossBar, TweenInfo.new(0.6), {BackgroundTransparency = 1}):Play()
			task.wait(0.7)
			alertFrame.Visible = false
	...
	-- Morph shop clicks (now that we have the events)
	for _, s in ipairs(shopSlots) do
		if s.item.cost then
			s.slot.MouseButton1Click:Connect(function()
				if ownedMorphs[s.item.name] then
					equipMorph:FireServer(s.item.name)
				else
					buyMorph:FireServer(s.item.name)
				end
			end)
		end
	end
	...
	-- Return to Lobby (settings button)
	...
	print("[LocalScript] Events connected")
end)

print("[LocalScript] GUI built")
